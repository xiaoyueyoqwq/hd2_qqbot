...
class DispatchService(APIRetryMixin):
    ...
    async def get_dispatches(self, limit: int = 5) -> Optional[List[Dict[str, Any]]]:
        ...
        try:
            # 优先从统一缓存管理器获取数据
            cached_dispatches = await hd2_cache_service.get_dispatches()
            
            if not cached_dispatches:
                bot_logger.warning("快讯缓存为空，数据正在后台更新中...")
                return None
    ...
    async def format_dispatch_messages(self, dispatches: List[Dict[str, Any]], start_index: int = 1) -> List[str]:
        """
        格式化快讯数据为多条消息（使用缓存翻译）
        
        Args:
            dispatches: 快讯数据列表
            start_index: 消息编号的起始数字
