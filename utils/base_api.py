...
class BaseAPI:
    ...
    # API 缓存的 TTL (秒) - 设置为0表示永不过期，始终使用缓存
    _cache_ttl: ClassVar[int] = 0  # 永不过期
    _cache_ttl_long: ClassVar[int] = 0  # 永不过期
                ...
                if use_cache_effective:
                    ...
                    if new_last_modified:
                        ...
                        if ttl > 0:
                            ...
                            # 永不过期缓存
                            await redis_manager.set(content_cache_key, response.content)
                            await redis_manager.set(lm_cache_key, new_last_modified)
                        bot_logger.debug(f"[BaseAPI] 响应已永久缓存, key: {content_cache_key}, lm_key: {lm_cache_key}")
                    else:
                        # 使用默认缓存 (永不过期)
                        ...
                        bot_logger.debug(f"[BaseAPI] 响应已永久缓存, key: {content_cache_key}")
