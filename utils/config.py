...
class Settings:
    ...
    # Debug 配置
    DEBUG_ENABLED = _config.get("debug", {}).get("enabled", True)
    DEBUG_TEST_REPLY = _config.get("debug", {}).get("test_reply", False)
    LOCAL_MODE = _config.get("debug", {}).get("local_mode", False)
    
    # 线程配置
    MAX_CONCURRENT = _config.get("max_concurrent", 5)  # 最大并发数
    MAX_WORKERS = _config.get("max_workers", 10)  # 最大工作线程数
    ...
    HD2_API_RETRY_INCREMENT = _config.get("hd2_api", {}).get("retry", {}).get("increment", 3.0)
    ...
    TRANSLATION_RETRY_INCREMENT = _config.get("translation", {}).get("retry", {}).get("increment", 3.0)
